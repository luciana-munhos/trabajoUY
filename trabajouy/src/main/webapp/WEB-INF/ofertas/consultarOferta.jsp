...
<body>
    ...
    String vencida = (String) request.getAttribute("vencida");
    boolean ya_postulo = pos != null;
    SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
	GregorianCalendar gregC;
	GregorianCalendar gregC2;
	Date date;
	Date fechaBaja;
	String fechaFormateada;
	String fechaBajaString;
	String vigencia_oferta = (String) request.getAttribute("vigencia");
	String favs = (String) request.getAttribute("favs");
	String finalizada = (String) request.getAttribute("finalizada");
	%>
    ...
    <div class="container-fluid mx-md-0 mx-auto">
            ...
            <div class="col-12 mx-auto col-md-5 m-0 ">
                <div class="container mt-md-0 mt-4 text-center text-md-end">
                    <div class="row border-bottom">
                        ...
                        %>
                        <p class="text-muted">Publicacion <a href="/trabajouy/consultarTipoPubli?nombreTP=<%=oferta.getTipoPublicacion() %>"><%=oferta.getTipoPublicacion() %></a></p>
                    </div>
                    <div class="row mt-2">
                        <div class="container">
                    ...
                    <%
                    if(favs==null && !(usr instanceof DtPostulante)){
                    	String mensaje="Marcada como favoritos por ";
        ...
        <%
        	if("true".equals(finalizada)){
    	        gregC2 = oferta.getFechaBaja().toGregorianCalendar();
    	        fechaBaja = gregC2.getTime();
    	        fechaBajaString = dateFormat.format(fechaBaja);
        	%>
        	<div class="row">
            	<p class="text-end text-muted">Dada de baja el <%=fechaBajaString %> por <a href="/trabajouy/consultarUsuario?nickname=<%=oferta.getNickEmpresa()%>"><%=oferta.getNickEmpresa()%></a></p>
        	</div>
            ...
            <div class="container-fluid m-0">
		        <div class="row">
		            ...
		            <%
                   		...
                   		String id = "cant:"+oferta.getNombre().replaceAll("\\s+","_");
               			%>
               			<div class="me-3 d-flex align-items-center text-center">
               			<span class="text-muted" id="<%=id%>">(<%=oferta.getCantFavoritos()%>)</span>
               			<%
               			id = "fav:"+oferta.getNombre().replaceAll("\\s+","_");
               			if(favs != null){
               			%>
		                            ...
		                            <div class="col container-fluid">
		                                <div class="row d-flex justify-content-center">
		                                    <div class="col">
		                                        ...
		                                        if (postulados.size() != 0) {
		                                        	//Me fijo si el primer postulante tiene orden asignado, de ser asi, todos tendrian y el ranking ya esta definido.     
		                                        	List<DtPostulacion> postus = (List<DtPostulacion>) request.getAttribute("postulaciones");
		                                        	DtPostulacion post1 = postus.get(0);
		                                        	if (post1.getOrden() == 0 && vigencia_oferta.equals("false")) { //En este caso la oferta no tiene ranking hecho y ya no esta vigente, se debe mostrar opcion de hacer ranking
		                                        		%>
		                                        		<div class="row mt-2">
		    		                                		<a class="btn btn-secondary" href="/trabajouy/realizarRanking?nombreOferta=<%=oferta.getNombre()%>">Seleccionar postulantes</a>                       
		    		                                	</div>
		    		                                <% 
		                                        	} else if (post1.getOrden() != 0 && vigencia_oferta.equals("false")){  //En este caso el orden no es 0, pues esta seteado y el ranking esta hecho, se muestra opcion de ver el ranking
		                                        		%>
		                                        		<div class="row mt-2">                    
	    		                                			<a class="btn btn-success" id="link_ranking" href="/trabajouy/exponerRanking?nombreOferta=<%=oferta.getNombre()%>">Ya has realizado el ranking. Toca para verlo.</a>                       
	    		                                		</div>          
		                                        	<%
		                                        	} else if (vigencia_oferta.equals("true")) { //En este caso la oferta sigue vigente y no debe dar la opcion de hacer el ranking, se debe esperar a que expire
		                                        	%>
		                                        	
		                                        	<div class="alert alert-primary" role="alert">
													  	¡Su oferta sigue publicada en nuestro sistema! 
													  	<br>
													  	Una vez haya caducado se le habilitara la seleccion de postulantes.
													</div>	
													<% 
		                                        	}
		                                        }
		            ...
		            <div class="modal-content">
		                ...
		                <div class="modal-body">
		                    <div class="container">
		                        ...
		                        <div class="row">
			                        <% if(!(postulacion.getVideo() == null) && !postulacion.getVideo().equals("")) {%>
				                        <div class="row">
				                        	<h5>Video de presentación</h5>
				                        	<div class="text-center">
				                        	<div class="video-container">
		                                    ...
		                                    <div class="col">
		                                        ...
		                                        if (postulados.size() != 0) {
		    		                                ...
		    		                                <% 
		                                        		...
		                                        		%>
		                                        		<div class="row mt-2">                    
	    		                                			<a class="btn btn-success" href="/trabajouy/exponerRanking?nombreOferta=<%=oferta.getNombre()%>">Ya has realizado el ranking. Toca para verlo.</a>                       
	    		                                		</div>          
		                                        	<%
		                                        	} else if (vigencia_oferta.equals("true")) { //En este caso la oferta sigue vigente y no debe dar la opcion de hacer el ranking, se debe esperar a que expire
		                                        	%>
    	...
    	i++;
             ...
             <div class="container pt-3" >
				            <div class="text-center pt-0">
				               <a type="button" class="mx-auto btn btn-lg" href="/trabajouy/consultarOferta?nombre=<%=oferta.getNombre()%>&Vencida=<%=false%>"
				                  style="background-color: #d87093;">Finalizar Oferta</a>
				            </div>
			            </div>
